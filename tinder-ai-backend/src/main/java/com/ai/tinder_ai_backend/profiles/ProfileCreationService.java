...
import static com.ai.tinder_ai_backend.Utils.generateMyersBriggsTypes;
import static com.ai.tinder_ai_backend.Utils.selfieTypes;
...
public class ProfileCreationService {
    ...
    public void createProfiles(int numberOfProfiles) {
        ...
        while (this.generatedProfiles.size() < numberOfProfiles) {
            ...
            String prompt = String.format("Create a Tinder profile persona of an %s %d year old %s %s including the first name, last name, Myers Briggs Personality type and a tinder bio. Save the profile using the saveProfile function",
                    personalityType, age, ethnicity, gender);
    ...
    private Profile generateProfileImage(Profile profile) {
        ...
        String prompt = String.format("Selfie of a %d year old %s %s %s, %s, photorealistic skin texture and details, individual hairs and pores visible, highly detailed, photorealistic, hyperrealistic, subsurface scattering, 4k DSLR, ultrarealistic, best quality, masterpiece. Bio- %s",
                profile.age(), profile.myersBriggsPersonalityType(), profile.ethnicity(), profile.gender(), randomSelfieType, profile.bio());
        ...
        String jsonString = String.format("{ \"prompt\": \"%s\", \"negative_prompt\": \"%s\", \"steps\":40 }", prompt, negativePrompt);
        System.out.println(String.format("Creating image for %s %s(%s)", profile.firstName(), profile.lastName(), profile.ethnicity()));
